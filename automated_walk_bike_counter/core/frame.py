	...
	def findDuplicateObjects(self):
		...
		for car1 in self.cars:
			for car2 in self.cars:
				if car1 is not car2:
					boxes_2compare = np.array([[car1.left,car1.top,car1.right,car1.bot],[car2.left,car2.top,car2.right,car2.bot]])
					o_rate = overlap_area(boxes_2compare)
					print("overlap: ", o_rate)
					if(o_rate > self.DUPLICATE_CAR_THREASHOLD):
						ped_boxes_dup_dict[car2] =1
						print("car exclude for car duplicates:",car2.left, car2.right, car2.top, car2.bot )

		for truck1 in self.trucks:
			for truck2 in self.trucks:
				if truck1 is not truck2:
					boxes_2compare = np.array([[truck1.left,truck1.top,truck1.right,truck1.bot],[truck2.left,truck2.top,truck2.right,truck2.bot]])
					o_rate = overlap_area(boxes_2compare)
					print("overlap: ", o_rate)
					if(o_rate > self.DUPLICATE_TRUCK_THREASHOLD):
						ped_boxes_dup_dict[truck2] =1
						print("truck exclude for truck duplicates:",truck2.left, truck2.right, truck2.top, truck2.bot )

		for car in self.cars:
			for truck in self.trucks:
				
				boxes_2compare = np.array([[car.left,car.top,car.right,car.bot],[truck.left,truck.top,truck.right,truck.bot]])
				o_rate = overlap_area(boxes_2compare)
				print("overlap: ", o_rate)
				if(o_rate > self.CAR_TRUCK_DUPLICATE_THREASHOLD):
					ped_boxes_dup_dict[truck] =1
					print("truck exclude for duplicates with car:",truck.left, truck.right, truck.top, truck.bot )

		for car in self.cars:
			for ped in self.pedestrians:
				boxes_2compare = np.array([[ped.left,ped.top,ped.right,ped.bot],[car.left,car.top,car.right,car.bot]])
				o_rate = overlap_area(boxes_2compare)
				print("overlap: ", o_rate)
				if(o_rate > self.DUPLICATE_THREASHOLD):
					ped_boxes_dup_dict[ped] =1
					print("Car excluded for duplication with pedestrian :",ped.left, ped.right, ped.top, ped.bot )
		...
		for nbox in listOfObjects:
			for mbox in listOfObjects:
				if(nbox.box == mbox.box):
					continue
				# check since inside conditions is valid for ped and bicycles and not for cars
				if not ((nbox.box[4]=="car" and mbox.box[4]=="person") or (nbox.box[4]=="person" and mbox.box[4]=="car")):

...
class ObjectTracker:
    ...
    def __init__(self, mask_image):
        ...
        self.object_classes = []
        self.object_costs = {'person':100,'car':200,'bus':300,'truck':200,'motorbike':120,'bicycle':100}
    ...
    def printDataReportOnFrame(self):
        if self.current_frame is not None:
            ...
            if 'person' in self.color_table:
                ...
                cv2.putText(self.current_frame.postprocessed_frame,counter,(x,y),cv2.FONT_HERSHEY_SIMPLEX,0.5, self.color_table['person'],2)
                x = x+gap
            if 'cyclist' in self.color_table:
                counter = "  Cyc:"+ str(self.object_counter.COUNTER_c)
                cv2.putText(self.current_frame.postprocessed_frame,counter,(x,y),cv2.FONT_HERSHEY_SIMPLEX,0.5, self.color_table['cyclist'],2)
                x = x+gap
            if 'car' in self.color_table:
                counter = "  Car:" + str(self.object_counter.COUNTER_car)
                cv2.putText(self.current_frame.postprocessed_frame,counter,(x,y),cv2.FONT_HERSHEY_SIMPLEX,0.5, self.color_table['car'],2)
                x = x+gap
            if 'bus' in self.color_table:
                counter = "  Bus:" + str(self.object_counter.COUNTER_bus)
                cv2.putText(self.current_frame.postprocessed_frame,counter,(x,y),cv2.FONT_HERSHEY_SIMPLEX,0.5, self.color_table['bus'],2)
                x = x+gap
            if 'truck' in self.color_table:
                counter = "  Truck:" + str(self.object_counter.COUNTER_truck)
                cv2.putText(self.current_frame.postprocessed_frame,counter,(x,y),cv2.FONT_HERSHEY_SIMPLEX,0.5, self.color_table['truck'],2)
                x = x+gap
            counter = "  Fr:" + str(self.currentFrameNumber)
            cv2.putText(self.current_frame.postprocessed_frame,counter,(x,y),cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,255,0),2)
    ...
    def addNewMovingObject(self,current_detected_object):
        ...
        print("New moving object added with the id of "+
            str(self.moving_object_id_number)+" detected as "+current_detected_object.mess+
            " in position : "+str(current_detected_object.left)+" "+str(current_detected_object.right)+" "+
            str(current_detected_object.top)+" "+str(current_detected_object.bot))
    ...
    def createContourForCurrentObjects(self,detected_objects):
        ...
        for obj in detected_objects:
            ...
            ##commented since time of testing car countters whihch was caused problem for ped counter
    ...
    def calculateCostMatrixForMovingObjects(self, currentDetectedObjects):
        def get_costs(curObject,cur_detected_objects):
            ...
            for obj in cur_detected_objects:
                ...
                dis = math.floor(math.sqrt((obj.center_x-curObject.predicted_position[-1][0])**2+(obj.center_y-curObject.predicted_position[-1][1])**2))
                dis += abs(self.object_costs[curObject.last_detected_object.mess]-self.object_costs[obj.mess])
                distances.append(dis)
                print("Distance between obj known "+str(obj.mess)+" at position "+str(obj.left)+" "+str(obj.right)+
                    " "+str(obj.top)+" "+str(obj.bot)+
                    " with object with id "+str(curObject.id)+" known as "+curObject.last_detected_object.mess+" is "+str(dis))
            return distances
        ...
        for index, obj in enumerate(self.lastFrameMovingObjects):
            ...
            if all(c > threshold for c in costs):
                print("object id "+str(obj.id)+" cost with all other detected objects is more than threshold and is counted as missing in kalman")
        ...
        with tf.Session(config=cfg) as sess:
            ...
            while camera.isOpened():
                ...
                if self.masked_image!=[]:
                    ...
                    img_ori = cv2.addWeighted(img_ori, 1-(self.output_video.opaque/100), mask_inv, (self.output_video.opaque/100), 0)
                else:
                ...
                if(len(self.lastFrameMovingObjects) == 0):
                    ...
                    if SaveVideo:
                        ...
                        self.update_frame_listener(self.current_frame.postprocessed_frame,self.currentFrameNumber)
                    if file == 0: #camera window
    ...
    def removeTrackedObjects(self,thresh):
        ...
        for index, obj in enumerate(self.lastFrameMovingObjects):
            ...
            if (obj.position[-1][0] < 0 or obj.position[-1][0] > w):
                ...
                print("Delete tracking object ", obj.position[-1][0], obj.position[-1][1],"out of the area of interest!}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}")
                del self.lastFrameMovingObjects[index]
    ...
    def predictMovingObjectsNewPosition(self,costMatrix,availableTrackedMovingObjects,cur_detected_objects):
        ...
        for detected_object_index,detected_object in enumerate(cur_detected_objects):
            ...
            if detected_object_index in indexes_np[:,1]:
                ...
                threshold = config.PED_COST_THRESHOLD
                if availableTrackedMovingObjects[tracked_obj_index].last_detected_object.mess == "bus":
                    threshold = config.BUS_COST_THRESHOLD
                elif availableTrackedMovingObjects[tracked_obj_index].last_detected_object.mess == "truck":
                    threshold = config.TRUCK_COST_THRESHOLD

                if costMatrix[tracked_obj_index][detected_object_index] > threshold:
                    print("Object id "+str(availableTrackedMovingObjects[tracked_obj_index].id)+" is going to add as a new object because of cost threshold")
                    self.addNewMovingObject(detected_object)
                    continue

                print("object "+str(availableTrackedMovingObjects[tracked_obj_index].id)+" has been assigned to object detected at "+
                                " position : "+str(cur_detected_objects[detected_object_index].left)+" "+str(cur_detected_objects[detected_object_index].right)+" "+
                                str(cur_detected_objects[detected_object_index].top)+" "+str(cur_detected_objects[detected_object_index].bot))
        ...
        for index,obj in enumerate(availableTrackedMovingObjects):
            if index not in indexes_np[:,0]:
                ...
                obj.kalman_update_missing(obj.predicted_position[-1])
                print("object id "+str(obj.id)+" has been disappeared in this frame")
        ...
        for index, obj in enumerate(self.lastFrameMovingObjects):
            ...
            # 		print(obj.last_detected_object.box[4]+" Delete tracking over max missing threshold")
            # 		del self.lastFrameMovingObjects[index]
    ...
    def checkObjectForDeletion(self,obj,index):
        ...
        if obj.frames_since_seen > config.MISSING_THRESHOLD:
            if obj.position[-1][0] < self.BOUNDRY or obj.position[-1][0] > self.video_width-self.BOUNDRY or obj.position[-1][1] < self.BOUNDRY or obj.position[-1][1] > self.video_height-self.BOUNDRY:
                ...
                print("object id: "+str(obj.id)+" frames_since_last_seen : "+str(obj.frames_since_seen))
                print(obj.last_detected_object.box[4]+" Delete tracking over max missing threshold")
                del self.lastFrameMovingObjects[index]

    def convertY3BoxesToBoxes(self,boxes_, scores_, labels_, originalImage, new_size, object_class):
    ...
    def drawBoxesOnFrame(self, obj):
        ...
        if mess=='person':
            if obj.id in self.object_counter.Pedestrians:
                plot_one_box(img_ori, [x0, y0, x1, y1], label="", color=self.color_table[mess])
            elif obj.id in self.object_counter.Cyclists:
                plot_one_box(img_ori, [x0, y0, x1, y1], label="Cyclist", color=self.color_table["cyclist"])
        elif mess == 'bicycle':
            print("Bicycle detected....")
        else:
            plot_one_box(img_ori, [x0, y0, x1, y1], label=mess, color=self.color_table[mess])
    ...
    def check_object_is_in_aoi(self,obj):
        ...
        r, g, b = self.masked_image[ int(obj.position[-1][0] * (self.image_processing_size[0]/self.video_width)), int(obj.position[-1][1]*(self.image_processing_size[1]/self.video_height)) ]

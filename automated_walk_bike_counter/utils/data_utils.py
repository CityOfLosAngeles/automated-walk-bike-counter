...
def parse_data(line, class_num, img_size, anchors, mode):
    ...
    if not isinstance(line, list):
        ...
        boxes = np.concatenate(
            (
                boxes,
                np.full(shape=(boxes.shape[0], 1), fill_value=1.0, dtype=np.float32),
            ),
            axis=-1,
        )
    else:
...
def get_batch_data(
    ...
    param:
        ...
        multi_scale: whether to use multi_scale training, img_size varies from
            [320, 320] to [640, 640] by default. Note that it will take effect only when
            mode is set to 'train'.
        interval: change the scale of image every interval batches. Note that it's
            indeterministic because of the multi threading.
    """
    global iter_cnt
        ...
        for idx, line in enumerate(batch_line):
            if np.random.uniform(0, 1) < 0.5:
                mix_lines.append(
                    [
                        line,
                        random.sample(batch_line[:idx] + batch_line[idx + 1 :], 1)[0],
                    ]
                )
            else:
    ...
    for line in batch_line:
        img_idx, img, y_true_13, y_true_26, y_true_52 = parse_data(
            line, class_num, img_size, anchors, mode
        )
